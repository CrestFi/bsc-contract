    ...
    ERC2771Recipient
{
    ...
    error CrestWalletAlreadyExists(bytes32 vault, address caller);
    error CrestWalletDoesNotExists(address caller);
    ...
    error InSufficientCrestWalletAmount(
        address token,
        uint256 requestAmount,
        uint256 vaultAmount
    );
    ...
    //Variables
    mapping(address => CrestWallet) public wallets;
    ...
    function _createCrestWallet(address walletAddress) internal {
        // Create a new Zebec wallet with the current timestamp as the creation time
        CrestWallet memory newWallet = CrestWallet({
        ...
        emit CreatedCrestWallet(walletAddress);
    }

    // Deposits tokens into a Zebec wallet.

    function depositTokens(
        address crestWalletAddress,
        uint256 amount,
        ...
        if (wallets[crestWalletAddress].createTime == 0)
            _createCrestWallet(crestWalletAddress);

        // Deposit the tokens into the Zebec wallet
        _depositToken(crestWalletAddress, amount, tokenAddress);
    }
    ...
    function _depositToken(
        ...
        if (tokenAddress == address(0)) {
            ...
            address owner = crestWalletAddress;
        ...
        walletTokenBalances[crestWalletAddress][tokenAddress] =
            walletTokenBalances[crestWalletAddress][tokenAddress] +
            (amount);
    ...
    function withdrawTokens(
        ...
        if (wallets[_msgSender()].createTime == 0)
            revert CrestWalletDoesNotExists(_msgSender());
        ...
        if (walletTokenBalances[_msgSender()][tokenAddress] < amount)
            revert InSufficientCrestWalletAmount(
                tokenAddress,
                amount,
    ...
    function _withdrawTokenInCrestWallet(
        address walletOwner,
        address receiver,
        uint256 amount,
        address tokenAddress
        ...
        emit CrestWalletWithdrawn(receiver, walletOwner, tokenAddress, amount);
    }
    ...
    function _instantTransfer(
        ...
        if (crestWalletWithdraw)
            _withdrawTokenInCrestWallet(sender, receiver, tokenAmount, token);
        ...
        isWhitelistedToken(streamingToken)
    {
        ...
        emit CreatedCrestWalletAndStreamed(
            _msgSender(),
            streamAddress,
            streamingToken,
            streamingAmount,
    ...
    function _createStream(
        ...
        CrestWallet storage senderWallet = wallets[streamSender];
        ...
        Stream memory currentStream = Stream({
            ...
            originCrestWallet: streamSender,
            canceled: false,
            paused: false,
    ...
    function withdrawStream(
        ...
        address originCrestWallet = currentStream.originCrestWallet;
        ...
        if (withdrawAmount > walletTokenBalances[originCrestWallet][token])
            revert InSufficientCrestWalletAmount(
                token,
                releasableAmount,
                walletTokenBalances[originCrestWallet][token]
            );
        ...
        _withdrawToken(originCrestWallet, address(staking), streamFee, token);

        // Perform the token withdrawal either in the Zebec wallet or directly to the receiver
        if (crestWalletWithdraw)
            _withdrawTokenInCrestWallet(
                originCrestWallet,
                receiver,
    ...
    function withdrawBulkTransferStream(
        ...
        withdrawStream(streamAddress, withdrawAmount, crestWalletWithdraw);
    ...
    function cancelStream(
        ...
        address originCrestWallet = stream.originCrestWallet;
        ...
        if (releasableAmount >= walletTokenBalances[originCrestWallet][token])
            revert InSufficientCrestWalletAmount(
                token,
                releasableAmount,
                walletTokenBalances[originCrestWallet][token]
    ...
    function pauseStream(
        ...
        Stream storage stream = streams[streamBytes];
        if (stream.originCrestWallet != _msgSender())
            revert InvalidOwner(_msgSender(), stream.originCrestWallet);
    ...
    function updateStream(
        ...
        Stream storage currentStream = streams[streamBytes];
        if (currentStream.originCrestWallet != _msgSender())
            revert InvalidOwner(_msgSender(), currentStream.originCrestWallet);
        ...
        emit UpdatedStream(
            streamBytes,
            streamName,
            currentStream.originCrestWallet,
            streamStartTime,
            streamEndTime,
            streamAmount
        );
    ...
    function getCrestWalletTokenBalance(
    ...
    function calculateStreamBytes(
        ...
        return keccak256(abi.encodePacked(crestWallet, _streamCount));
    }
        ...
        )
    {
        ...
        for (uint256 i; i < wallets[crestWallet].streamCount; ++i) {
            // Iterate over the token addresses
            for (uint256 j; j < tokenAddressLength; ++j) {
                // Calculate the unique stream bytes
                bytes32 streamBytes = calculateStreamBytes(crestWallet, i);
        ...
        for (uint256 j; j < tokenAddressLength; ) {
            if (
                outgoingStreamBalance[j] <=
                walletTokenBalances[crestWallet][tokenAddress[j]]
            ) {
